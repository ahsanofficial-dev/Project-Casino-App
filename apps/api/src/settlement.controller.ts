...
export class SettlementController {
  ...
  async settle(
    ...
    @Param('outcome') outcome: 'WON' | 'LOST' | 'VOID',
    @Headers('x-sandbox-admin-key') key?: string,
  ) {
    if (!process.env.SANDBOX_ADMIN_KEY || key !== process.env.SANDBOX_ADMIN_KEY) {
      throw new BadRequestException('Sandbox settlement authorization required');
    }
      ...
      if (payout > 0) {
        ...
        await tx.ledgerEntry.create({
          data: {
            userId: bet.userId,
            amountCents: payout,
            type: outcome === 'VOID' ? 'WITHDRAWAL_REVERSAL' : 'BET_PAYOUT',
            idempotencyKey: `settlement:${bet.id}:${outcome}`,
            referenceId: bet.id,
          },
        });
      }
