...
export class WalletController {
  ...
  @Post('mock-deposit') async deposit(@Req() req: Request, @Body() dto: AmountDto, @Headers('idempotency-key') rawKey?: string) {
  ...
  @Post('mock-withdrawal') async withdraw(@Req() req: Request, @Body() dto: AmountDto, @Headers('idempotency-key') rawKey?: string) {
    const u = await this.current.require(req); const key = requireIdempotencyKey(rawKey);
    const existing = await this.prisma.payment.findUnique({ where: { idempotencyKey: key } });
    if (existing && (existing.userId !== u.id || existing.amountCents !== dto.amountCents || existing.type !== 'WITHDRAWAL')) throw new BadRequestException('Idempotency key was already used');
    const payment = existing ?? await this.prisma.payment.create({ data: { userId: u.id, type: 'WITHDRAWAL', amountCents: dto.amountCents, status: 'COMPLETED', providerReference: `mock_${key}`, idempotencyKey: key } });
